...
function LoadSettings()
    if readfile and writefile and isfile and isfolder then
        ...
        if not isfile("Unique Hub Premium Scripts/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json") then
            writefile("Unique Hub Premium Scripts/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", game:GetService("HttpService"):JSONEncode(getgenv().Config))
        else
...
spawn(function()
    game:GetService("RunService").Heartbeat:Connect(function()
        pcall(function()
            for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                if (Auto_Farm_Bone or getgenv().Config.Mastery["Farm Mastery SwordList"]) and StatrMagnetBoneMon and (v.Name == "Reborn Skeleton [Lv. 1975]" or v.Name == "Living Zombie [Lv. 2000]" or v.Name == "Demonic Soul [Lv. 2025]" or v.Name == "Posessed Mummy [Lv. 2050]") and (v.HumanoidRootPart.Position - PosMonBone.Position).magnitude <= 350 then
    ...
    while task.wait() do
        pcall(function()
            if _G.Setting_table.Bring_Mob then
                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if (Auto_Spikey_Trident or getgenv().Config.Mastery["Farm Mastery SwordList"] or _G.Setting_table.SWOW) and MagnetDought then
...
getgenv().Config.Mastery["Select Multi Swordd"] = AllSwordInInventroy
ms:MultiDropdown("Select Sword",AllSwordInInventroy,getgenv().Config.Mastery["Select Multi Swordd"],function(vu)
    getgenv().Config.Mastery["Select Multi Swordd"] = vu
    SaveSettings()
end)
...
ms:Toggle("Auto Farm Mas Sword",getgenv().Config.Mastery["Farm Mastery SwordList"],function(vu)
    getgenv().Config.Mastery["Farm Mastery SwordList"] = vu
    SaveSettings()
    spawn(function()
        while wait() do
            pcall(function()
                if getgenv().Config.Mastery["Farm Mastery SwordList"] and #getgenv().Config.Mastery["Select Multi Swordd"] ~= 0 then
                    for i,v in pairs(getgenv().Config.Mastery["Select Multi Swordd"]) do
                        if getgenv().Config.Mastery["Farm Mastery SwordList"] == false and table.find(SwordListFarmComplete,v) then
                            break;
                        end
                        if not game.Players.LocalPlayer.Backpack:FindFirstChild(v) and not game.Players.LocalPlayer.Character:FindFirstChild(v) and game.Players.LocalPlayer.Character:WaitForChild("Humanoid"):GetState() ~= Enum.HumanoidStateType.Dead and game.Players.LocalPlayer.Character:WaitForChild("Humanoid").Health ~= 0 then
                            while getgenv().Config.Mastery["Farm Mastery SwordList"] do wait()
                                if game.Players.LocalPlayer.Character:WaitForChild("Humanoid").Health == 0 then 
                                    repeat wait() 

                                    until game.Players.LocalPlayer.Character:WaitForChild("Humanoid").Health > 0; wait(0.2) 
                                end
                                if game.Players.LocalPlayer.Backpack:FindFirstChild(v) or game.Players.LocalPlayer.Character:FindFirstChild(v) or getgenv().Config.Mastery["Farm Mastery SwordList"] == false or table.find(SwordListFarmComplete,v) then 
                                    break;
                                end
                                wait(0.5)
                        ...
                        if game.Players.LocalPlayer.Backpack:FindFirstChild(v) or game.Players.LocalPlayer.Character:FindFirstChild(v) then
                            while getgenv().Config.Mastery["Farm Mastery SwordList"] do wait()
                                if table.find(SwordListFarmComplete,v) and #getgenv().Config.Mastery["Select Multi Swordd"] ~= 0 and #SwordListFarmComplete ~= 0 then
                                    break
                                end
                                if getgenv().Config.Mastery["Farm Mastery SwordList"] == false then
                                    break;
                                end
                                if game.Players.LocalPlayer.Backpack:FindFirstChild(v) or game.Players.LocalPlayer.Character:FindFirstChild(v) then
                                    SelectWeaponInSwordList = v
